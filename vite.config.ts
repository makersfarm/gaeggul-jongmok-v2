...
import { getFinancialTrend } from "./server/dart";
        ...
        configureServer(server) {
          ...
          server.middlewares.use("/api/analysis", async (req, res) => {
            try {
              ...
              const [financialTrend, technical] = await Promise.all([
                getFinancialTrend(stock.code, 5),
                getTechnicalSnapshot(stock.code),
              ]);
              json(res, buildAnalysis(stock, financialTrend, technical));
            } catch (error) {
